...
const ApiForm = () => {
  ...
  const [brc20Loading, setBrc20Loading] = useState(false); // BRC20のローディング状態を管理
  const [ordinalsData, setOrdinalsData] = useState(null); // Ordinalsデータを保存
  const [brc20PresetData, setBrc20PresetData] = useState([]); // BRC20プリセットデータを保存
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (type === 'ordinals') {
        setOrdinalsData(data); // Ordinalsのデータを保存
      }
      setResult(data);
    } catch (error) {
  ...
  const fetchOrdinalsData = async () => {
    ...
    console.log(`Fetching Ordinals data from URL: ${url}`);
    ...
    try {
      ...
      const data = await response.json();
      console.log('Fetched Ordinals data:', data);
      setOrdinalsData(data);
    } catch (error) {
      console.error('Error fetching Ordinals data:', error, 'for URL:', url);
      setResult({ error: `Error fetching Ordinals data for URL: ${url}` });
    }

    setLoading(false); // ローディングを終了
  };

  const fetchBrc20PresetData = async (preset) => {
    setBrc20Loading(true); // ローディングを開始
    const results = [];
    ...
    setBrc20Loading(false); // ローディングを終了
    setBrc20PresetData(results); // BRC20プリセットデータを保存
  };

  const handlePresetSelect = async (preset) => {
    setSelectedPreset(preset);
    if (type === 'ordinals') {
      await fetchOrdinalsData();
    } else if (type === 'brc20') {
      await fetchBrc20PresetData(preset);
    }
  };

  const handleFetchClick = () => {
    if (type === 'ordinals' && ordinalsData) {
      const presetItems = ordinalsPresets.ordinals;
      const filteredData = presetItems.map(presetItem => {
        return ordinalsData.find(item => item.collectionSymbol.toLowerCase() === presetItem.collectionSymbol.toLowerCase());
      }).filter(Boolean); // 存在する銘柄のみ抽出
      setResult(filteredData);
    } else if (type === 'brc20' && brc20PresetData.length > 0) {
      setResult(brc20PresetData);
    }
  };
  ...
  return (
    <Box sx={{ maxWidth: '100%', overflowX: 'auto' }}>
      ...
      <form onSubmit={e => { e.preventDefault(); fetchData(); }}>
        ...
        <Box marginY={2}>
          <Button variant="contained" color="primary" type="submit" disabled={loading || brc20Loading}>
            {loading || brc20Loading ? <CircularProgress size={24} /> : "Fetch Data"}
          </Button>
          <Button variant="contained" color="secondary" onClick={saveDataAsImage} style={{ marginLeft: '10px' }} disabled={loading || brc20Loading}>
            Save Data as Image
          </Button>
          <Button variant="contained" color="secondary" onClick={() => saveDataToSheet(result)} style={{ marginLeft: '10px' }} disabled={loading || brc20Loading}>
            Save Data to Google Sheets
          </Button>
          <Button variant="contained" color="primary" onClick={handleFetchClick} style={{ marginLeft: '10px' }} disabled={loading || (!ordinalsData && !brc20PresetData.length)}>
            Fetch Filtered Data
          </Button>
        </Box>
      </form>
